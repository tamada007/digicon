		...
		if True:
			...
			for flb_row in flb_rows:
				...
				if flb_row[enum.FLBField.CHECK_DIGIT][0] & 0x80 == 0:
					...
					iBarType = 1
				if iBarType == 0 and strBarcodeType == "ITF": continue
				elif iBarType == 1 and strBarcodeType == "EAN": continue
			...
			if iBarType == 1: newLineData[enum.FLBField.CHECK_DIGIT][0] |= 0x80
			else: newLineData[enum.FLBField.CHECK_DIGIT][0] &= ~0x80
	...
	def easySendPlu(self, plu_json_file):
		...
		for data_line in json_data:
			...
			new_plu_row[enum.PLUField.PLU_STATUS_2][0] = \
					master.call_list["BIN"]().csv_to_cell("00000000 00000000 10000000", 3, plumt.fields_info_dic["PLUStatus2"])
			new_plu_row[enum.PLUField.PLU_STATUSB][0] = \
					master.call_list["BIN"]().csv_to_cell(" ".join(["00000000"] * 4), 4, plumt.fields_info_dic["PLUStatus1B"])
			new_plu_row[enum.PLUField.PLU_STATUS_2B][0] = \
					master.call_list["BIN"]().csv_to_cell(" ".join(["00000000"] * 8), 8, plumt.fields_info_dic["PLUStatus2B"])
			...
			if oSpecialMessage:
				...
				if len(specialMessageLinesArr) != len(specialMessageLinesFontArr):
					...
					if isinstance(oSpecialMessageFont, (str, unicode)):
						specialMessageLinesFontArr = [ const.fontMapper.get(oSpecialMessageFont, 0) ] * len(specialMessageLinesArr)
					else:
			...
			for i in range(5):
				...
				if oText:
					...
					src_str_comp = '\r\n'.join([str(text_line["font_size"]) + "," + "\"" + text_line["text"] + "\"" for text_line in combTextLines])
