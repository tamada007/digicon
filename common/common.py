...
def close_all_sqlite_db():
	if hasattr(g_current_thread_data, "Database") and isinstance(g_current_thread_data.Database, dict):
		for key in g_current_thread_data.Database:
			#print "key1:", key
			try: g_current_thread_data.Database[key].close()
			except: pass
		g_current_thread_data.Database.clear()
		
	for key in g_memConn:
		#print "key2:", key
		try: g_memConn[key].close()
		except:pass
	g_memConn.clear()
...
def log_init():
	...
	Rthandler = logging.handlers.RotatingFileHandler('digicon_detail.log', mode='a', maxBytes=5*1024*1024, backupCount=5)
	Rthandler.setLevel(logging.DEBUG)
	#formatter = logging.Formatter('%(asctime)s : %(message)s', datefmt='%a, %d %b %Y %H:%M:%S')
	formatter = logging.Formatter('%(asctime)s : %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
	Rthandler.setFormatter(formatter)
	logging.getLogger('detail').addHandler(Rthandler)
	...
	try:
		...
		pass
	Rthandler = logging.handlers.RotatingFileHandler('digicon_result.log', mode='w', maxBytes=5*1024*1024, backupCount=5)
	Rthandler.setLevel(logging.DEBUG)
	#formatter = logging.Formatter('%(asctime)s : %(message)s', datefmt='%a, %d %b %Y %H:%M:%S')
	formatter = logging.Formatter('%(asctime)s : %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
	Rthandler.setFormatter(formatter)
	logging.getLogger('result').addHandler(Rthandler)
